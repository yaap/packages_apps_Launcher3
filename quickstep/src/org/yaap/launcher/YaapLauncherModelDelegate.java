...
public class YaapLauncherModelDelegate extends QuickstepModelDelegate
    ...
    public void onTargetsAvailable(List<SmartspaceTarget> targets) {
        List<SmartspaceTarget> list = targets.stream().filter(t -> t.getFeatureType() != 34).collect(Collectors.toList());
    ...
    public void workspaceLoadComplete() {
        ...
        mSmartspaceSession = ((SmartspaceManager) mContext.getSystemService(SmartspaceManager.class)).createSmartspaceSession(new SmartspaceConfig.Builder(mContext, "home").build());
